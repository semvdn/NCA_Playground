...
1.  **Clone the repository (or create the files as provided):**
    ```bash
    git clone https://github.com/semvdn/NCA_Playground
    cd NCA_Playground
    ```
...
1.  Navigate to the root directory of the project (`NCA_Playground/`).
...
*   **Simulation Controls:**
    ...
    *   **Step Back:** Reverts the NCA grid to the previous state (history is limited).
...
*   **NCA Parameters:**
    ...
    *   **Weight Scale:** Adjust the initial magnitude of the neural network's weights.
    *   **Bias:** Adjust the initial bias values in the neural network.
    *   **Hidden Layer Sizes:** Dynamically add or remove hidden layers and set their node counts. The input layer is fixed at 9 (for the 3x3 neighborhood) and the output layer at 1 (for the new cell state).
    *   **Apply General Settings:** Applies changes made to Preset, Colormap, Activation, Weight Scale, Bias, and Hidden Layer Sizes. May randomize weights if the network structure changes.
*   **Randomization:**
    *   **Randomize Weights:** Randomizes all network weights based on current general configuration (keeps the grid state).
    *   **Randomize Grid:** Generates a new random grid state (keeps the current network weights).
*   **Manual Neuron Weight Editor:**
    *   **Target Layer/Neuron:** Select a specific layer and neuron to inspect or modify its incoming weights. You can also select "All Neurons" in a layer to apply a pattern.
    *   **Load Preset Pattern:** Apply predefined weight patterns (e.g., Zeros, Ones, Center On) to the selected neuron(s).
    *   **Individual Weight Inputs:** For a single selected neuron, manually adjust each incoming weight.
    *   **Apply Manual Weights:** Applies the specified weights or pattern to the selected neuron(s). This clears the simulation history.

### Right Panel (Under the Hood)

*   **Cell & Network Inspector:**
    *   **Cell Info:** Displays information about the currently selected cell from the CA canvas.
        ...
        *   **Neighborhood:** The 3x3 grid of values surrounding the selected cell, including the cell itself. These 9 values are fed into the neural network as input.
        *   **Layer Activations:** Shows the intermediate values (pre-activation `z` and post-activation `h`) for each layer of the MLP during the forward pass for the selected cell. This illustrates how the network transforms the input to produce the next cell state.
    *   **Clear Selection:** Clears the currently selected cell and its details.
*   **Network Visualization Canvas:**
    *   Displays a dynamic diagram of the Multi-Layer Perceptron (MLP) used by the NCA.
    ...
    *   When a neuron is selected in the "Manual Neuron Weight Editor", it is highlighted in the network visualization.
